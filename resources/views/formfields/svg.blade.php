@if(isset($dataTypeContent->{$row->field}))
    <div class="svg-wrap" data-field-name="{{ $row->field }}">
        ...
        <input type="hidden" name="{{ $row->field }}_uploaded" value="{{ $dataTypeContent->{$row->field} }}">
    </div>
@endif
<input @if($row->required == 1 && !isset($dataTypeContent->{$row->field})) required @endif type="file" name="{{ $row->field }}" accept="image/svg">
