...
class ThumbsController extends Controller
{
	...
	public static function getSourceDisk(){
		return config('voyager.storage.disk', 'public');
	}
	
    public static function getThumbsDisk(){
		return config('voyager.thumbs.storage.disk', self::getSourceDisk());
	}
	...
	public static function createThumbnail($thumbModel, $img_path, $table, $folder, $id, $field, $mark, $filename, $ext, $is_gallery = false)
	{
		...
		if(config('voyager.thumbs.saving')){
			Storage::disk(self::getThumbsDisk())->put($path, $encoded);
		}
	...
	public function generatePlaceholder($mark, $ext)
	{
		...
		if(config('voyager.thumbs.saving')){
			Storage::disk(self::getThumbsDisk())->put($path, $image->toJpeg());
		}
	...
	public static function getAbsPath($path){
		return urldecode(Storage::disk('s3')->url(self::getRelPath($path)));
	}
}
